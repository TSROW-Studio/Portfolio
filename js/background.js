...
// Mobile Detection - Disable WebGL on mobile for performance
const isMobile = window.innerWidth < 768 || /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);

if (isMobile) {
    // Hide canvas on mobile, show static gradient instead
    const canvas = document.getElementById('gl-canvas');
    if (canvas) {
        canvas.style.display = 'none';
    }
    // Expose empty API to prevent errors
    window.BG_ENGINE = {
        setScrollVelocity: () => { },
        setMood: () => { },
        setMoodValue: () => { }
    ...
    // Desktop - Initialize full WebGL experience
